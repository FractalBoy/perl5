...
#define USE_UNDERLYING_LOCALE   NULL
...
/* The next layer up to catch vagaries and bugs in the libc setlocale return
 * value */
#ifdef stdize_locale
...
S_less_dicey_setlocale_r(pTHX_ const int category, const char * locale)
{
    ...
    /* The call above may already have placed its return in PL_setlocale_buf */
    if (retval != PL_setlocale_buf) {
...
#    define void_setlocale_c(cat, locale)                                   \
                          void_setlocale_i(cat##_INDEX_, locale)
#    define void_setlocale_r(cat, locale)  void_setlocale_c(cat, locale)    \
       void_setlocale_i(get_category_index(cat, locale), locale)
...
#    ifdef LC_ALL
    ...
    if (category == LC_ALL) {
        ...
        if (! made_changes) {
            ...
            retval = save_to_buffer(calculate_LC_ALL(individ_locales),
                                    buf, buf_size);
        }
...
#  endif
        ...
        /* Note that we didn't have to savepv(); set_numeric_standard() doesn't
         * do any locale manipulation. */
...
#if defined(USE_LOCALE) && (   defined(USE_LOCALE_MONETARY) \
...
#  endif
    ...
    if (cat_index == LC_NUMERIC_INDEX_) {
        ...
        return my_langinfo_i(item, cat_index, USE_UNDERLYING_LOCALE,
                             &PL_langinfo_buf, &PL_langinfo_bufsize, utf8ness);
    }
...
S_my_langinfo_i(pTHX_
                 ...
                 * USE_UNDERLYING_LOCALE  => Use the current underlying locale
                 * of the program
...
/* The second version of my_langinfo() is if we have plain nl_langinfo() */

    {
        ...
        else if (locale != USE_UNDERLYING_LOCALE) {
    ...
    if (locale == USE_UNDERLYING_LOCALE) {
        ...
        if (strNE(want_locale, querylocale_i(cat_index))) {
            const char * orig_switched_locale = toggle_locale_i(cat_index,
                                                                want_locale);
            retval = my_langinfo_i(item, cat_index, USE_UNDERLYING_LOCALE,
...
#  else
    ...
    {   /* We save the current value for each category; switch to the global
        ...
        for (i = 0; i < NOMINAL_LC_ALL_INDEX; i++) {
            PERL_UNUSED_RESULT(save_to_buffer(querylocale_i(i),
                                              &curlocales[i], NULL));
        }
